 ...
 */
        ...
        public:
            inline DiscriminativeBit(uint16_t const significantByteIndex, uint8_t const existingByte,
        ...
        private:
            ...
            getByteRelativeSignificantBitIndex(uint8_t const existingByte, uint8_t const newKeyByte);

        };

        inline DiscriminativeBit::DiscriminativeBit(uint16_t const significantByteIndex, uint8_t const existingByte,
                                                    uint8_t const newKeyByte)
        ...
        inline uint16_t
        DiscriminativeBit::getByteRelativeSignificantBitIndex(uint8_t const existingByte, uint8_t const newKeyByte) {
            uint32_t mismatchByteBitMask = existingByte ^ newKeyByte;
            return __builtin_clz(mismatchByteBitMask) - 24;
        }

        template<typename Operation>
        inline bool executeForDiffingKeys(uint8_t const *existingKey, uint8_t const *newKey, uint16_t keyLengthInBytes,
        ...
        template<typename InsertOperation, typename UpdateOperation>
        inline bool executeForDiffingKeys(uint8_t const *existingKey, uint8_t const *newKey, uint16_t keyLengthInBytes,
                                          InsertOperation const &insertOp, UpdateOperation const &updateOp) {
            for (size_t index = 0; index < keyLengthInBytes; ++index) {
                uint8_t newByte = newKey[index];
                uint8_t existingByte = existingKey[index];
                if (existingByte != newByte) {
                    insertOp(DiscriminativeBit{static_cast<uint16_t>(index), existingByte, newByte});
                    return true;
                }
            }
            //keys matching, insert duplicate into existing tid span
            updateOp();
            return false;
        }

        inline idx::contenthelpers::OptionalValue<DiscriminativeBit>
        getMismatchingBit(uint8_t const *existingKey, uint8_t const *newKey, uint16_t keyLengthInBytes) {
            for (size_t index = 0; index < keyLengthInBytes; ++index) {
                uint8_t newByte = newKey[index];
                uint8_t existingByte = existingKey[index];
                if (existingByte != newByte) {
